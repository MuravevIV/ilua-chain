...
--
-- (!) Modified version to fix issue with EmmyLua's "Lua Remote(Mobdebug)" debugging from-Windows-to-WSL2 on Windows 10.
--
-- Usage:
-- # 1. Enable the general fix and override iscasepreserving
-- export MOBDEBUG_WSL_PATH_FIX="true"
-- # 2. Set your Windows project root that corresponds to /mnt/c/...
-- # Ensure it has a trailing slash!
-- export MOBDEBUG_WINDOWS_PROJECT_ROOT="C:/projects/project-name/"
-- # 3. Enable dot-to-slash conversion for module paths if IntelliJ sends them
-- export MOBDEBUG_MODULE_DOT_TO_SLASH="true"
--
-- Original: https://raw.githubusercontent.com/pkulchenko/MobDebug/4cf97f9555c8e30e189eb53034daf773bd619955/src/mobdebug.lua
-- For modified code fragments, search for:
-- MODIFICATION_mod_1
-- REMOVED_mod_1
--
...
-- Environment variable controls for custom path handling
local wsl_path_fix_enabled = os.getenv("MOBDEBUG_WSL_PATH_FIX")
wsl_path_fix_enabled = wsl_path_fix_enabled == "true" or wsl_path_fix_enabled == "1"

local windows_project_root_env = os.getenv("MOBDEBUG_WINDOWS_PROJECT_ROOT") -- e.g., "C:/projects/ilua-stream/"
if windows_project_root_env and windows_project_root_env:sub(-1) ~= "/" then
    windows_project_root_env = windows_project_root_env .. "/" -- Ensure trailing slash
end

local module_dot_to_slash_enabled = os.getenv("MOBDEBUG_MODULE_DOT_TO_SLASH")
module_dot_to_slash_enabled = module_dot_to_slash_enabled == "true" or module_dot_to_slash_enabled == "1"

if wsl_path_fix_enabled then
    print("MOBDEBUG: WSL Path Fix ENABLED.")
    if windows_project_root_env then
        print("MOBDEBUG: Windows Project Root for stripping: '" .. windows_project_root_env .. "'")
    end
    if module_dot_to_slash_enabled then
        print("MOBDEBUG: Module dot-to-slash conversion ENABLED.")
    end
end
...
print("MOBDEBUG_INIT: Original iscasepreserving detection = " .. tostring(iscasepreserving))

if wsl_path_fix_enabled then
    iscasepreserving = true -- Override for WSL specific fixes
    print("MOBDEBUG_INIT: WSL Path Fix forcing iscasepreserving = true")
end
...
local function set_breakpoint(file, line)
    ...
    if processed_file == '-' and lastfile then
        processed_file = lastfile
    else
        if wsl_path_fix_enabled then
            processed_file = string.gsub(processed_file, "\\", "/")
            if windows_project_root_env then
                local lower_processed_file = string.lower(processed_file)
                local lower_win_project_root = string.lower(windows_project_root_env)
                if string.sub(lower_processed_file, 1, #lower_win_project_root) == lower_win_project_root then
                    processed_file = string.sub(processed_file, #lower_win_project_root + 1)
                end
            end
            if module_dot_to_slash_enabled then
                if not string.find(processed_file, "/") and string.find(processed_file, "%.") then
                    if not string.match(string.lower(processed_file), "^[a-z]:") and not string.match(processed_file, "%.lua$") then
                        processed_file = string.gsub(processed_file, "%.", "/")
                    end
                end
            end
        ...
        if iscasepreserving then
            processed_file = string.lower(processed_file)
        end
    end
    if not breakpoints[line] then breakpoints[line] = {} end
    breakpoints[line][processed_file] = true
end
...
local function remove_breakpoint(file, line)
    ...
    if processed_file == '-' and lastfile then
        processed_file = lastfile
    elseif processed_file == '*' and line == 0 then
        breakpoints = {}
        return
    else
        if wsl_path_fix_enabled then
    ...
    if breakpoints[line] then
        breakpoints[line][processed_file] = nil
        if next(breakpoints[line]) == nil then
...
local function has_breakpoint(file, line)
    ...
    if wsl_path_fix_enabled then
        if string.sub(file_to_check, 1, 2) == "./" then
            file_to_check = string.sub(file_to_check, 3)
        end
    end

    if iscasepreserving then
        file_to_check = string.lower(file_to_check)
    else
        if wsl_path_fix_enabled then
            file_to_check = string.lower(file_to_check)
        end
    ...
    local found = breakpoints[line][file_to_check]
    if not found and wsl_path_fix_enabled and breakpoints[line] then
        local alternate_file_to_check
        if string.sub(file_to_check, -4) == ".lua" then
            alternate_file_to_check = string.sub(file_to_check, 1, -5)
        else
            alternate_file_to_check = file_to_check .. ".lua"
        end
        found = breakpoints[line][alternate_file_to_check]
    end
    return found
end
