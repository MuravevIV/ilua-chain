...
describe("Chain:leftJoin()", function()

    it("performs left join with single matches", function()
        local left = {
            {id = 1, name = "Alice"},
            {id = 2, name = "Bob"},
            {id = 3, name = "Charlie"}
        ...
        local resFn = function(l, r)
            return {name = l.name, score = r and r.score or nil}
        end

        local result = chain(left):leftJoin(chain(right), lKeyFn, rKeyFn, resFn):toList()
    ...
    it("performs left join with multiple matches on one key", function()
        local left = {
            {id = 1, name = "Alice"},
            {id = 2, name = "Bob"}
        }
    ...
    it("includes left when no matches", function()
        local left = {
            {id = 1, name = "Alice"}
        }
        local right = {
    ...
    it("handles empty left chain", function()
        ...
        local rKeyFn = function(el, k, i) return el.id end
        local resFn = function(l, r)
            return {score = r and r.score or nil}
        end
    ...
    it("uses index in key extractors", function()
        ...
        expect(result).to.equal({"ax", "az", "by", "cx", "cz", "dy"})
    end)
    ...
    it("uses key in key extractors", function()
        ...
        table.sort(parts)
        expect(parts).to.equal({"a", "bx", "by", "cz"})
    end)
    ...
    it("throws error if rChain is not a Chain instance", function()
        ...
        local resFn = function(l, r) return l + (r or 0) end

        expect(function()
            chain(left):leftJoin(notChain, lKeyFn, rKeyFn, resFn)
    ...
    it("throws error if lKeyExtFn is not a function", function()
        ...
        expect(function()
            chain(left):leftJoin(right, lKeyFn, rKeyFn, resFn)
        end).to.fail("leftJoin(rChain, lKeyExtFn, rKeyExtFn, resSelFn): lKeyExtFn must be a function (got string: 'not function')")
    end)
    ...
    it("throws error if rKeyExtFn is not a function", function()
        ...
        expect(function()
            chain(left):leftJoin(right, lKeyFn, rKeyFn, resFn)
        end).to.fail("leftJoin(rChain, lKeyExtFn, rKeyExtFn, resSelFn): rKeyExtFn must be a function (got string: 'not function')")
    end)
    ...
    it("throws error if resSelFn is not a function", function()
        ...
        expect(function()
            chain(left):leftJoin(right, lKeyFn, rKeyFn, resFn)
        end).to.fail("leftJoin(rChain, lKeyExtFn, rKeyExtFn, resSelFn): resSelFn must be a function (got string: 'not function')")
    end)
end)
