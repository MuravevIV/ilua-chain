...
local function serialize_table(t)
  ...
  local parts = {}
  -- Use ipairs for clean sequence serialization
...
local paths = {
  ...
  equal = {
    test = function(actual, ...)
      local expected = {...}
      if #expected > 1 then
        -- Multi-value mode: compare a table of actuals with a table of expecteds
        local actual_str = serialize_table(actual)
        local expected_str = serialize_table(expected)
        return strict_eq(actual, expected),
        'expected values ' .. actual_str .. ' to exactly equal ' .. expected_str,
        'expected values ' .. actual_str .. ' to not exactly equal ' .. expected_str
      else
        -- Original single-value mode
        local expected_val = expected[1]
        return strict_eq(actual, expected_val),
        'expected ' .. tostring(actual) .. ' and ' .. tostring(expected_val) .. ' to be exactly equal',
        'expected ' .. tostring(actual) .. ' and ' .. tostring(expected_val) .. ' to not be exactly equal'
      end
    end
  },
  have = {
    ...
    test = function(v, errmsg)
      ...
      if errmsg ~= nil then
        ...
        if success then
          ...
          suffix = 'error message: "' .. msg .. '"'
        end
        return not success and #msg >= #errmsg and msg:sub(-#errmsg) == errmsg,
        'expected ' .. tostring(v) .. ' to fail with message substring: "' .. errmsg .. '", got ' .. suffix,
        'expected ' .. tostring(v) .. ' to not fail with message substring: "' .. errmsg .. '", got ' .. suffix
      else
        return not success,
